...
class SPNRPBuilder(object):
    ...
    def __init__(self, data,spn_object=None,ds_context=None,leaves_size=8000,scope=None,threshold=0.4,prob=0.7,ohe=True,proportion=0.2,**kwargs):
        ...
        self.target_shape=kwargs['target_shape']
...
class spatialtree(object):
    ...
    def split(self, data,**kwargs):
        ...
        if len(self.scope) == 1 or len(self.__indices)<self.leaves_size :
    ...
    def build_spn_leaf_sum(self, data,scope,**kwargs):
        ...
        for i in range(0,len(scope)):
            ...
            if isinstance(node,Categorical):
                temp=list();
                for i in range(0,self.target_shape):
                    temp.append(0)
                temp[int(self.target_index)]=1
                node.p=temp
