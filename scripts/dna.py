...
for train_index, test_index in kf.split(credit):
    ...
    original = time.time()
    #spn_classification =  learn_parametric(numpy.array(X),ds_context,min_instances_slice=10000,threshold=0.1)
    ...
    print('Building tree...')
    original = time.time();
    T = SPNRPBuilder(data=numpy.array(X),ds_context=ds_context,target=X,prob=0.7,leaves_size=2,height=3,spill=0.9)
