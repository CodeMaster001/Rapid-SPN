...
credit = fetch_openml(name='dna', version=1,return_X_y=True)[0]
...
for train_index, test_index in kf.split(credit):
    ...
    original = time.time()
    spn_classification =  learn_parametric(numpy.array(X),ds_context,min_instances_slice=1000000,threshold=0.1)
    ...
    print('Building tree...')
    original = time.time();
    T = SPNRPBuilder(data=numpy.array(X),ds_context=ds_context,target=X,prob=0.7,leaves_size=2,height=4,spill=0.9)
...
os.makedirs("results/dna")
numpy.savetxt('results/dna/ours.time', ours_time_list, delimiter=',')
numpy.savetxt('results/dna/theirs.time',theirs_time_list, delimiter=',')
numpy.savetxt('results/dna/theirs.ll',theirs, delimiter=',')
numpy.savetxt('results/dna/ours.ll',ours, delimiter=',')
