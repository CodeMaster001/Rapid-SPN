...
def optimize_tf_graph(
    ...
    with tf.Session(config=config) as sess:
        ...
        while  True:
            ...
            if np.abs(old_loss) < 0.0002 or i>epochs:
               break;
            i = i +1
...
credit = fetch_openml(name='ecoli', version=1,return_X_y=True)[0]
...
for train_index, test_index in kf.split(credit):
    ...
    original = time.time()
    spn_classification =  learn_parametric(numpy.array(X),ds_context,min_instances_slice=20,threshold=0.2)
   ...
   # print(ll_test)
    ...
    original = time.time();
    T = SPNRPBuilder(data=numpy.array(X),ds_context=ds_context,target=X,prob=0.2,leaves_size=4,height=3,spill=0.3)
...
os.makedirs("results/ecoli")
numpy.savetxt('results/ecoli/ours.time', ours_time_list, delimiter=',')
numpy.savetxt('results/ecoli/theirs.time',theirs_time_list, delimiter=',')
numpy.savetxt('results/ecoli/theirs.ll',theirs, delimiter=',')
numpy.savetxt('results/ecoli/ours.ll',ours, delimiter=',')
