...
for instance in [25]:
    credit= fetch_openml(name='labor', version=1,return_X_y=True)[0]
    credit = pd.DataFrame(data=credit)
    credit = credit.apply(LabelEncoder().fit_transform)

    kf = KFold(n_splits=credit.shape[0],shuffle=True)
    theirs = list()
    ours = list()
    ours_time_list = list()
    ...
    #parameters
    output_file_name='labor.'+str(instance)+'.oneall.log'
    ...
    for train_index,test_index in kf.split(credit):
        X_train,X_test=credit.values[train_index],credit.values[test_index]
