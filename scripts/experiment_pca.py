...
def spnrp_train(X,X_test,context,height=2,prob=0.5,leaves_size=20,bandwidth=0.2,epochs=1000,selector_array=[2,3,4],threshold=1,use_optimizer=True,predict_bandwidth=0.4):
    try:
        ...
        print(ll_test)
        test_data_mpe = X.astype(float)
        test_data_mpe[:,:] = np.nan
        print(os.getcwd())
        f=open(OBJECT_DIR+'obj'+str(height)+str(leaves_size),'wb')
        print('called')
        mpe_result = mpe(spn,test_data_mpe)
        np.save(f,mpe_result)
        f.flush();
        f.close();
