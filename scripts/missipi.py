...
credit=pd.read_csv("../dataset/isolet.data",delimiter=',') 
...
for train_index, test_index in kf.split(credit):
    ...
    X_test = np.load("X_test.npy")#credit.values[test_index,:];	
    ...
    T = SPNRPBuilder(data=X,ds_context=ds_context,prob=0.5,leaves_size=40,height=3,min_items=40,spill=0.25,samples_rp=1,target_index=i,target_shape=X.shape[1])
    T= T.build_spn();
    T.update_ids();
    spn = T.spn_node;
    ...
    cor = np.corrcoef(X_test[:,440:])
    cor_pred = np.corrcoef(X_test_eval[:,440:])
    plt.matshow(cor)
    plt.savefig("cor_1.jpg")
    plt.matshow(cor_pred)
    plt.savefig("cor_2.jpg")
    print(ll_test)
    print("--ll--")
    #print(numpy.mean(ll_test_original))
    print(numpy.mean(ll_test))
    if np.isinf(numpy.mean(ll_test)):
        np.save("X.npy",X)
