...
train_dataset=np.append(train_dataset,np.array(train_dataset[:,-1]).reshape(-1,1),axis=1)
...
for i in range(0,train_dataset.shape[1]-2):
...
for height in [4,6,8,12,14,16,18,20,22,24,26,30]:
    for leaves_size in [2,4,6,8,10,12,14,16,20,40,60,80,100]:
        ...
        P=subprocess.Popen(['./experiment_acc.py train.npy test.npy context.npy '+opt_args.strip()],shell=True)
        P.communicate()
        P.wait();
        P.terminate()
