...
credit = fetch_openml(name='creditcard', version=1,return_X_y=True)[0]
...
for train_index, test_index in kf.split(credit):
    ...
    original = time.time()
    spn_classification =  learn_parametric(numpy.array(X),ds_context,min_instances_slice=40,threshold=0.7)
   ...
   # print(ll_test)
    ...
    original = time.time();
    T = SPNRPBuilder(data=numpy.array(X),ds_context=ds_context,target=X,prob=0.9,leaves_size=4,height=2,spill=0.2)
    ...
    spn=optimize_tf(spn,X,epochs=2000,batch_size=1000)
