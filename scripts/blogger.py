...
for train_index, test_index in kf.split(credit):
    ...
    original = time.time()
    spn_classification =  learn_parametric(numpy.array(X),ds_context,min_instances_slice=20,threshold=0.4)
   ...
   # print(ll_test)
    ...
    spn_mean=numpy.mean(ll_test)


    logging.info('Building tree...')
    original = time.time();
    T = SPNRPBuilder(data=numpy.array(X),ds_context=ds_context,target=X,prob=0.6,leaves_size=1,height=1,spill=0.7)
