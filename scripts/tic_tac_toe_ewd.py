...
train_dataset,labels= fetch_openml(name='tic-tac-toe', version=1,return_X_y=True)
le = LabelEncoder()
train_dataset = train_dataset[train_dataset.columns].apply(le.fit_transform)
train_dataset = train_dataset.sample(72)
...
#         X_train,X_test= train_test_split(train_dataset,random_state=42)
#         run_execution_character(X_train=X_train,X_test=X_test,min_instance_slice=min_instance_slice,height=height,leaves_size=leaves_size,threshold=threshold,output_file_name=output_file_name)
