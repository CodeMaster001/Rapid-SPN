...
credit = fetch_openml(name='hepatitis', version=1,return_X_y=True)[0]
...
for train_index, test_index in kf.split(credit):
    ...
    #l_test = eval_tf(spn_classification, X_test)
    #print(ll_test)
    ...
    T = SPNRPBuilder(data=numpy.array(X),ds_context=ds_context,target=X,prob=0.6,height=3,leaves_size=100,spill=0.4)
...
os.makedirs("results/flares")
numpy.savetxt('results/flares/ours.time', ours_time_list, delimiter=',')
numpy.savetxt('results/flares/theirs.time',theirs_time_list, delimiter=',')
numpy.savetxt('results/flares/theirs.ll',theirs, delimiter=',')
numpy.savetxt('results/flares/ours.ll',ours, delimiter=',')
