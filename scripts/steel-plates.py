...
for instance in [50000]:
    credit= fetch_openml(name='steel-plates-fault', version=1,return_X_y=True)[0]
    ...
    #parameters
    output_file_name='steel.'+str(instance)+'.'+str(int(sys.argv[1]))+'.log'
    ...
    threshold =0.2
    bandwidth=0.5
    predict_bandwidth=0.8

    opt_args= str(output_file_name) + ' ' + str(min_instances_slice) +' ' +str(epochs) + ' '+ str(height) + ' '+str(prob) + ' ' +str(leaves_size)+' ' + str(threshold)+ ' '+str(predict_bandwidth) + ' 0'

    for i in range(0,credit.shape[1]):
        context.append(Gaussian)
    for train_index,test_index in kf.split(credit):
        X_train,X_test=credit[train_index],credit[test_index]
