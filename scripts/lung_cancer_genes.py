...
credit = fetch_openml(name='lungcancer_GSE31210', version=1,return_X_y=True)[0]
#/credit = np.ones(shape=(100000,10))
credit = np.delete(credit,1, 0)  
print(credit.shape)
...
for train_index, test_index in kf.split(credit):
    ...
    original = time.time()
    spn_classification =  learn_parametric(numpy.array(X),ds_context,min_instances_slice=30,threshold=0.4)
    ...
    print('Building tree...')
    original = time.time();
    T = SPNRPBuilder(data=numpy.array(X),ds_context=ds_context,target=X,prob=0.2,leaves_size=2,height=4,spill=0.5)
    ...
    spn=optimize_tf(spn,X,epochs=60000,optimizer= tf.train.AdamOptimizer(0.0000001))
