...
train_dataset,labels= fetch_openml(name='CIFAR_10',cache=True, version=1,return_X_y=True)
...
output_file_name='cifar_pca_'+str(sys.argv[2])+'_'+str(sys.argv[3])+'.log'
epochs=8000
height=1
prob=0.5
leaves_size=30
...
for instance_slice in [10,20,30,40,50,100]:
    opt_args= str(output_file_name) + ' ' + str(instance_slice) +' ' +str(height) + ' '+str(leaves_size)+' '+str(threshold) 
    P=subprocess.Popen(['python3 experiment_pca.py train.npy test.npy context.npy '+opt_args.strip()],shell=True)
    P.communicate()
    P.wait();
    P.terminate()
