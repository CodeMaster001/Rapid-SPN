...
for i in range(10):
    ...
    print('Building tree...')
    original = time.time();
    T = SPNRPBuilder(data=numpy.array(X),ds_context=ds_context,target=X,prob=0.3,leaves_size=2,height=2,spill=0.7)
    ...
    spn=optimize_tf(spn,X,epochs=5000,optimizer= tf.train.AdamOptimizer(0.001))
