...
credit,target = fetch_openml(name='kr-vs-k', version=1,return_X_y=True)
...
for train_index, test_index in kf.split(credit):
    ...
    spn_classification =  learn_parametric(numpy.array(X),ds_context,threshold=0.3,min_instances_slice=30)
    ...
    print('Building tree...')
    original = time.time();
    T =  SPNRPBuilder(data=numpy.array(X),ds_context=ds_context,leaves_size=20,target=X.shape[1]-1,rule='rp',height=2,prob=0.8,spill=0.76)
