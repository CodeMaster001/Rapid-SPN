...
theirs = list()
ours = list();
credit,target = fetch_openml(name='micro-mass', version=1,return_X_y=True)
...
for train_index, test_index in kf.split(credit):
    ...
    spn_classification =   learn_parametric(X,ds_context, min_instances_slice=10000)
    ...
    print('Building tree...')
    original = time.time();
    T =  SPNRPBuilder(data=X,ds_context=ds_context,target=X,leaves_size=2,height=3,samples_rp=20,prob=0.40,spill=0.25)
